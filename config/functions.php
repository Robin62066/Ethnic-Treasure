...
function user_url($file = '')
{
    global $config;
    $url = $config['base_url'];
    return $url . $config['user_folder'] . '/' . $file;
}
...
function do_upload($name)
{
    ...
    if (isset($_FILES[$name]['name']) && $_FILES[$name]['name'] != '') {
        ...
        $file_name = date("Y/m/") . time() . '-' . bin2hex(random_bytes(10)) . '.' . $extension;
        $target_path =  ROOT_PATH . upload_dir($file_name);
...
function render(bool $success, $message = null,  $data = null,  int $response_code = 200)
{
